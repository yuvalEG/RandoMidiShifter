//
//  RandoMidiShifterExtensionDSPKernel.hpp
...
#import "RandoMidiShifterExtension-Swift.h"
#import "RandoMidiShifterExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 RandoMidiShifterExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class RandoMidiShifterExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in RandoMidiShifterExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case RandoMidiShifterExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case RandoMidiShifterExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<RandoMidiShifterExtensionDSPKernel *>(context);
